...
\begin{lstlisting}[caption={Leis para $Monad$\protect\footref{fnt:haskell-language-report}}, style=haskell-style, label={lst:monad-laws}]
return a >>= k = k a
m >>= return = m
m >>= (\x -> k x >>= h) = (m >>= k) >>= h
\end{lstlisting}
...
\begin{lstlisting}[caption={Definição do tipo $Kleisli$\protect\footref{fnt:prog-with-arrows}}, style=haskell-style, label={lst:kleisli-type}]
...
\begin{lstlisting}[caption={Instância para $Monad$\protect\footref{fnt:prog-with-arrows}}, style=haskell-style, label={lst:kleisli-arrow}]
...
\begin{lstlisting}[caption={Leis para $Arrow$\protect\footref{fnt:arrows-and-idioms}}, style=haskell-style, label={lst:arrow-law}]
arr id >>> f = f
f >>> arr id = f
(f >>> g) >>> h = f >>> (g >>> h)
arr (g . f) = arr f >>> arr g
...
Recentemente obteve-se o suporte a funções anônimas. Estas foram adicionadas a partir do Lambda Project~\cite{jsr335}, na versão 8 da JDK. Esta funcionalidade é essencial para a implementação de um contrato monádico, pois acrescenta características equivalentes as já existentes em linguagens como Haskell. Logo, contando com as funcionalidades já citadas e as recém adicionadas na plataforma, Java é capaz de fornecer suporte a mónades e flechas.

\subsection{Problema dos Tipos Construtores}

Para a construção da interface monádica, precisa-se primeiro eliminar algumas das inconsistências da linguagem. Todo mónade opera sob o tipo construtor ${M\!\!\!<\!\!\!A\!\!>}$. Logo, a declaração abaixo é suficientemente capaz de satisfazer as necessidades de um contrato monádico .

\begin{lstlisting}[caption={Interface Monad\protect\footref{fnt:arrows-and-idioms}}, style=haskell-style, label={lst:arrow-law}]
public interface Monad<M<A>> {}
\end{lstlisting}

Entretanto, o compilador não suporta a sintaxe ${M\!\!\!<\!\!\!A\!\!>}$. Tipos genéricos não são capazes de expressar tipos construtores. Portanto, para expressar esta interface faz-se necessário a implementação de $Kind$.

\begin{lstlisting}[caption={Interface Kind}, style=haskell-style, label={lst:kind-interface}]
public interface Kind<A, _ extends Kind<?, _>> {}
\end{lstlisting}

Conforme a Listagem~\ref{lst:kind-interface}, $Kind$ é capaz de simular tipos construtores parcialmente.

Afim de simular tipos construtores, fez-se necessária a construção da interface $Kind$. Apesar da assinatura confusa, esta interface define dois tipos genéricos. São eles:

\begin{enumerate}[label=(\alph*), leftmargin=3em, topsep = 0pt, itemsep = 1ex, partopsep = 1ex, parsep = 1ex]
	\item $A$ tomando como exemplo o tipo construtor invalido ${M<B>}$, este primeiro parâmetro genérico eh equivalente ao valor encapsulado $B$ nesta declaração.
	\item $\textunderscore$ tal parâmetro unifica A e M, ou seja, determina uma restrição na qual $\textunderscore$ herda $Kind$ e parametriza este com um tipo curinga e o próprio $\textunderscore$ que representa M.
\end{enumerate}

Apesar de impor uma restrição hierárquica entre os tipos genéricos em sua declaração, $Kind$ consegue simular tipos construtores em algumas funções. Com a nova inferência de tipos do compilador Java, esta interface garante 
...
interface Monad [listagem monad] foi utilizada para representar a type class da listagem x. Ambas, type class e interfaces, nomeiam e descrevem um conjunto de operações, a partir de entradas e saídas. Contudo, estas diferenciam entre si, pois uma type class limita o comportamento de um tipo enquanto uma interface limita o comportamento de uma classe. Além disso, para definir o contrato monádico eh necessário declarar também o contrato de functor, visto que todo monad é um functor.
