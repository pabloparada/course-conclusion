...
Conforme a Listagem~\ref{lst:func-io}, as funções de E/S retornam valores encapsulados na mónade $IO$. Estes retornos são incompatíveis, seus tipos de retorno não alinham. Assim, para permitir a composição destes tipos de funções, Hughes introduziu a type class $Arrow$.

\begin{lstlisting}[caption={Funções de E/S\protect\footref{fnt:arrow-user-interface}}, style=haskell-style, label={lst:tc-arrow}]
class Arrow a where
	arr   :: (b -> c) -> a b c
	(>>>) :: a b c -> a c d -> a b d
...
Como pode ser visto na Listagem~\ref{lst:tc-arrow}, $Arrow$ denota uma computação com os tipos de entrada $c$ e $d$ e tipos saída $b$ e $d$, um tipo construtor $a$ e as operações $arr$, $>>>$ e $first$~\cite{courtney2001genuinely}.

\addtocounter{footnote}{1}
\footnotetext{Fonte: A. Courtney e E. Conal, Genuinely functional user interfaces, p. 5, 2001.\label{fnt:arrow-user-interface}}

\addtocounter{footnote}{1}
\footnotetext{Fonte: J. Hughes, Programming with Arrows, p. 73--74, 2005.\label{fnt:prog-with-arrows}}
