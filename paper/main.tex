...
Portando, busca-se fornecer novas funcionalidades a partir de recursos pré-existentes na linguagem Java. O suporte a lambda expressions e as estruturas advindas da teoria das categorias dão origem a soluções para problemas cada vez mais presentes na atualidade.

Com o incremento na inferência de tipos do compilador tais estruturas podem ser simplificadas. A simulação de tipos construtores e as complicadas hierarquias de tipos devem ser substituídas por soluções nativas. Como possível trabalho futuro têm-se diversas outras estruturas oriundas da Teoria das Categorias que ainda carecem de implementação.
