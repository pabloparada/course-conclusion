...
Com as instancia de $Eq \; Int$ é possível comparar dois inteiros tal que ${equals({1 + 1}, 2)}$ retorna $True$; ou ${equals({1 + 1}, 0)}$ retorna $False$. De maneira similar, com $Eq \; Char$ é possível comparar dois caracteres, onde ${equals('a', {'a'})}$ retorna $True$; ou ${equals('a', {'b'})}$ retorna $False$.

\subsection{A Type Class Monad -- Construção Fundamental}

\textcolor{red}{<descrever a type class monad, mostrar exemplos de utilização e linkar com a TC>}

\subsection{A Type Class Arrow -- Generalização de Mónades}
