...
Assim como $Monad$, subclasses de $Arrow$ devem utilizar seus construtores para incluir $B \rightarrow C$ em seus contextos. Além disso, o método $andThen$ define a composição de $Arrow$ por $A(B, D) = A(B, C) \; andThen \; A(C, D)$.

\subsection{Implementação de Kleisli}

Para qualquer mónade existe uma flecha correspondente, tal que funções do tipo $A \rightarrow M(B)$ são flechas em potencial.
