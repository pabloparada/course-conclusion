...
\subsection{Functor -- Morfismos entre Categories}
...
Estas funções capazes de preservar as características das categories $C$ e $D$ também podem ser ilustradas a partir da figura~\ref{fig:func-f}.
...
Podendo ser visto como um padrão estrutural que ocorre diversos ramos da matemática~\cite{spivak2014cts}, a construção fundamental chamada monad estrutura-se a partir de um endofunctor~\footnote{Um functor que mapeia uma categoria para ela mesma.} e das transformações naturais~\footnote{Mapeamento entre dois functors que possuem o mesmo domínio e contradomínio, tal que satisfaça a condição de naturalidade.} de identidade e multiplicação.\\

\textbf{Definição}: o monad $T = (T, \eta, \mu)$ em uma category $C$ consiste em:
...
Fazendo com que os diagramas~\ref{fig:mon-comut-ident} e~\ref{fig:mon-comut-assoc} comutem, respeitando os axiomas de identidade e associatividade.
...
\centering
	...
	\end{tikzpicture}
	\caption{Identidade à esquerda e direita}
...
Triplas de Kleisli são equivalentes a mónades, oferecendo uma outra forma representação sintática. Nesta, objetos ainda estão contidos na categoria original, mas seus morfismos fazem parte da Categoria de Kleisli, habilitando a de composição destes.
