...
\centering
	...
	\end{tikzpicture}
\caption{Funções entre coleções de objetos~\cite{awodey2010category}}
...
\begin{figure}[h]
	\begin{minipage}[t]{.45\linewidth}
		\centering
			...
			\end{tikzpicture}
			\caption{Identidade~\cite{awodey2010category}}
	...
	\begin{minipage}[t]{.53\linewidth}
		\centering
			...
			\end{tikzpicture}
			\caption{Associatividade~\cite{awodey2010category}}
...
\begin{figure}[h]
			...
			\end{tikzpicture}
			\caption{Funtor $F$~\cite{awodey2010category}}
			\label{fig:func-f}
\end{figure}
...
\centering
	...
	\end{tikzpicture}
	\caption{Identidade à Esquerda e à Direita~\cite{maclane1971mat}}
	...
	\begin{tikzpicture}[baseline=-0.8ex]
		\matrix (m) [
			...
			\path[->]
				...
				(m-1-2) edge node[right] {$\mu$} (m-2-2);
	\end{tikzpicture}
	\caption{Associatividade~\cite{maclane1971mat}}
...
Apesar de demonstrar a implementação apenas para o tratamento de nulidade, mónades podem de fato servir como abstrações para concorrência. Uma implementação concreta da interface mónade pode encapsular funções e executa-las em um contexto contendo threads. Desta forma, o pacote de concorrência disponível em Java pode abstrair funções de baixo nível e trabalhar com uma interface genérica que facilite a construção de programas concorrentes.

Em ambientes distribuídos, muitas das vezes, faz-se necessário compartilhar valores entre diferentes máquinas. Outra importante implementação desta interface são as mónades de leitura e escrita. De forma similar a mónade de concorrência, tais mónades encapsulam funções que trabalham com estes valores e abstraem a complexidade do ambiente contido nestas aplicações.

Por meio do polimorfismo obtido na implementação deste contrato, $Kleisli$ consegue generalizar a composição de todas as subclassificações de mónade. Esta instância permite a execução de funções de forma sequencial em ambos contextos monádicos, respeitando seus axiomas e mantendo suas estruturas. Portanto, é possível encadear diferentes computações através de diferentes mónades.

Dessa forma, incrementou-se o suporte ao paradigma funcional
na plataforma Java. A composição destas construções tornaram-se polimórficas, expressadas pela instância $Kleisli$ e generalizadas pela interface $Monad$. Este mesmo polimorfismo também é oferecido para desenvolvedores que desejam criar bibliotecas, referenciando apenas interfaces padronizadas.

Com o incremento na inferência de tipos do compilador, no futuro tais implementações podem ser simplificadas. Outras estruturas oriundas da Teoria das Categorias carecem de implementação e podem novamente incrementar a plataforma.
