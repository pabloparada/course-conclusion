...
\lstdefinestyle{java-style}{basicstyle = \footnotesize\tt, breakatwhitespace = true, breaklines = true, captionpos = b, extendedchars = true, language = Java, frame = single, keywordstyle = \bf, showspaces = false, showstringspaces = false, showtabs = false, tabsize = 2, abovecaptionskip = 1em, belowcaptionskip = 1em, aboveskip = 2em,     belowskip = 5pt, xleftmargin = 5pt, xrightmargin = 5pt}

\lstdefinestyle{haskell-style}{basicstyle = \footnotesize\tt, breakatwhitespace = true, breaklines = true, captionpos = b, extendedchars = true, language = haskell, frame = single, keywordstyle = \bf, showspaces = false, showstringspaces = false, showtabs = false, tabsize = 2, abovecaptionskip = 1em, belowcaptionskip = 1em, aboveskip = 2em,     belowskip = 5pt, xleftmargin = 5pt, xrightmargin = 5pt}
...
Na Listagem~\ref{lst:instance-eq}, instancia-se $Eq$ para os tipos $Int$ e $Char$. A primeira instancia declara que $Int$ pertence a $Eq$, e que a implementação de igualdade de inteiros é dada por $primEqInt$. Similarmente, $Char$ pertence $Eq$ e fornece a implementação de igualdade por $primEqChar$. Logo, como $a$ é um parametro de tipo, as assinaturas de $primEqInt$ e $primEqChar$ são tipadas ${Int \rightarrow Int \rightarrow Bool}$ e ${Char \rightarrow Char \rightarrow Bool}$ consecutivamente~\cite{hall1996type}.
...
A operação $return$ encapsula o tipo $a$ no contexto monádico $m\;a$ e retorna este como resultado. A próxima é bind, declarada simbolicamente por $>>=$, que habilita o encadeamento de funções. Bind aplica a função $a \rightarrow m\;b$ no contexto monádico $m\;a$, retornando um novo $Monad$ do tipo $m\;b$.
...
\begin{lstlisting}[caption={Monad - Associatividade e Identidade\protect\footref{fnt:haskell-language-report}}, style=haskell-style, label={lst:monad-laws}]
return a >>= k  =  k a
m >>= return  =  m
m >>= (\x -> k x >>= h)  =  (m >>= k) >>= h
\end{lstlisting}
...
\begin{lstlisting}[caption={Encadeando Funções em Monad\protect\footref{fnt:haskell-language-report}}, style=haskell-style, label={lst:func-chaining}]
putStr "x: " >>
getLine >>= \l -> 
return (words l)
\end{lstlisting}
...
\begin{lstlisting}[caption={Do Notation\protect\footref{fnt:haskell-language-report}}, style=haskell-style, label={lst:do-notation}]
do 
	putStr "x: "
	l <- getLine
	return (words l)
...
\addtocounter{footnote}{1}
\footnotetext{Fonte: S. Marlow \textit{et al.}, Haskell 2010 Language Report, p. 25--81, 2010. Disponível em: \url{http://www.haskell.org/onlinereport/haskell2010}. Acessado em: 23/09/2015.\label{fnt:haskell-language-report}}

\subsection{Classe Arrow}

\textcolor{red}{<descrever a type class arrow, mostrar exemplos de utilização e linkar com a TC>}
