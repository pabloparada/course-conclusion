...
O conceito de flecha abstrai uma computação e generaliza mónade. Esta permite a definição de uma computação parcialmente estática, geralmente que dependa do seus tipos de entrada. A instância chamada Flecha de Kleisli fornece o operador de composição monádica. Seu funcionamento é análogo a mónade, onde valores são encapsulados em flechas e modificados pela aplicação de funções em seu contexto.
