...
A interface $Monad$, conforme~\ref{lst:monad-interface}, conta com as operações monádicas de identidade e multiplicação através do construtor de classe e do método $flatMap$. O tipo $Kind$ também está presente, assim como seus parâmetros de tipo definidos em $A$ e $M$. Também é importante salientar a implementação de $Functor$ que mantém consistente a hierarquia dessas construções e garante a implementação de $map$.

\begin{lstlisting}[float = *, caption = {Leis para $Arrow$\protect\footref{fnt:arrows-and-idioms}}, label = {lst:monad-interface}]
...
Com a interface definida é possível construir as diversas implementações de mónade como em Haskell. Como exemplo, na Listagem~\ref{lst:maybe-class} define-se o mónade $Maybe$.

\begin{lstlisting}[float = *, caption = {Interface $Maybe$\protect\footref{fnt:arrows-and-idioms}}, label = {lst:maybe-class}]
...
Esta interface fornece a implementação padrão dos métodos $flatMap$ e $map$. As subclasses $Just$ e $Nothing$ funcionam como construtores, representando instâncias concretas de $Maybe$.
