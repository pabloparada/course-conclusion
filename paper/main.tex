...
Com a rápida adoção de mónades para a solução de problemas envolvendo E/S, constatou-se que outros conceitos da TC também poderiam ser aproveitados. Qualquer estrutura recursiva (listas, mapas, árvores, grafos) que pode ser mapeada a partir de uma função é representada por um funtor. A categoria de Haskell, chamada Hask, preocupa-se em tratar tipos como objetos e funções como morfismos, fornecendo funções de composição e identidade.
...
\centering
	...
	\bigskip
	\caption[Typeclass Hierarchy]{Hierarquia das type classes\footnotemark}
	\label{fig:typeclassopedia}
\end{figure}
\footnotetext{Fonte:~\url{https://wiki.haskell.org/Typeclassopedia}. Acessado em 17/09/2015.}
...
A implementação de uma type class pode ser visualizada na Listagem~\ref{lst:type-class-functor}, onde a primeira declaração incorpora uma nova classe com nome Functor, uma variável de tipo f e um comportamento explicitado pela função fmap. Na segunda, a instanciação de Maybe fornece a implementação de fmap, tornando este tipo um Functor.

\begin{lstlisting}[caption={Type class para funtores\protect\footnotemark\label{fnt:type-class-funtor}}, style=haskell-style, label={lst:type-class-functor}]
...
\end{lstlisting}
\footnotetext{Fonte: M. Lipovaca,~\textit{Learn You a Haskell for Great Good!: A Beginner's Guide}, 2011.\label{fnt:type-class-funtor}}

A Listagem~\ref{lst:fmap-maybe} demonstra a utilização da função fmap em conjunto do variante Just com valor 200 e a função de multiplicação parcialmente aplicada.

\begin{lstlisting}[caption={Função fmap para Maybe\protect\footnotemark\label{fnt:type-class-funtor}}, style=haskell-style, label={lst:fmap-maybe}]
fmap (*2) (Just 200)  
\end{lstlisting}

\textcolor{red}{<mostrar a type class de monad e explicar type constructors>}\\
